...
class BERT_SA_TE(nn.Module):
    def __init__(self, bert, opt):
        ...
        self.dense1 = nn.Linear(opt.bert_dim, opt.class_dim)
        self.dense2 = nn.Linear(opt.bert_dim,opt.class_dim)

    def forward(self, inputs):
        text_bert_indices, bert_segments_ids = inputs[0], inputs[1]
        encoded_layer, label_layer= self.bert(text_bert_indices, bert_segments_ids, output_all_encoded_layers=False)
        output1 = self.dropout1(encoded_layer)
        output2=self.dropout2(label_layer)
